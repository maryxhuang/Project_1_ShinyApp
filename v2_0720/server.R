shinyServer(function(input, output){
  ...
  yearly_info <- reactive({
    ushr[complete.cases(ushr), ] %>% 
      filter(., Year == input$year_selected & Crime.Solved == "Yes") %>% 
      filter(., Perpetrator.Age > 5 & Victim.Age < 99)})
  
  age_spe_data <- yearly_info %>% 
    select(., Perpetrator.Age, Victim.Age) %>% 
    summarise(., avg_pep_age = mean(Perpetrator.Age), avg_vic_age = mean(Victim.Age), 
              age18_pep = , age18_vic = )
