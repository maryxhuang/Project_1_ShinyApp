shinyServer(function(input, output){
  ...
  age_year_info_per <- reactive({
    ushr %>% 
      filter(., Year == input$year_selected & Crime.Solved == "Yes") %>% 
      select(., Perpetrator.Age) %>% 
      filter(., Perpetrator.Age > 5) %>% 
      group_by(., Age = Perpetrator.Age) %>% 
      summarise(., age_per_count = n()) %>% 
  ...
  # sex_all_data$Incident <- factor(sex_all_data$Incident, 
                                  # levels = sex_all_data$Incident[order(sex_all_data$Percentage)])
  output$sex_all_pie <- renderPlot({
    ggplot(data = sex_all_data, aes(x = 1, y = Percentage)) +
  ...
  output$wp_all_fillperc <- renderPlot({
    ggplot(weapon_all_data, aes(x = Year, y = Incidence, group = Weapon.Type, fill = Weapon.Type)) +
